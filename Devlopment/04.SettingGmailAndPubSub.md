# 📬 Gmail Push Notifications via Google Cloud – KT & Implementation Guide
...
-   **Solution**: Immediately after creating the authenticated Gmail client, we make a call to the `users.getProfile` endpoint. This lightweight call returns the user's current `historyId`, which we then save as the starting point for our first email sync.
    ...
    // Get the most recent historyId to avoid missing emails
    const profile = await gmail.users.getProfile({ userId: "me" });
    const initialHistoryId = profile.data.historyId;

    // Now, proceed with setting up the watch and storing the historyId
    await startWatch(gmail);
    ```
...
### Challenge 4: Incorrect Database Queries
-   **Problem**: The `isAuthenticated` middleware was failing because it used `UserModel.findById(decoded.email)`. The `findById` method expects a MongoDB ObjectId, but the JWT payload contained the user's email address (a string).
-   **Log Signature**: `WARN: ❌ JWT verification failed` (caused by the subsequent database error).
-   **Solution**: The query was corrected to use `UserModel.findOne({ email: decoded.email })`. This ensures we are querying the `users` collection using the correct field and data type, allowing the authentication check to succeed.
